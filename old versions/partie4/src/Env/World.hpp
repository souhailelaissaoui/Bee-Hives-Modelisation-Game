...
public:
    ...
    void drawOn(sf::RenderTarget& target) const; // dessine le monde dans le parametre target
    ...
    void saveToFile(); // sauvegarde le monde actuel dans un fichier .map
    bool isGrowable(const Vec2d& p); // verifie si la cellule dans laquelle se trouve la position graphique passée en argument est une cellule d'herbe.
    ...
    World();
    void clamping(Vec2d& center);

    bool isHiveable(Vec2d const& position, double radius);
    std::vector<std::size_t> indexesForRect(CellCoord const& topLeft, CellCoord const& bottomRight) const;
