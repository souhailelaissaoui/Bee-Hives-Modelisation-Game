...
void World::AddWater(Vec2d pos)
{
    int x1 = pos.x / sizecell_;
    int y1 = pos.y / sizecell_;
    if (isInsideWorld(x1, y1) and (cells_[nbcells_ * y1 + x1] != Kind::Eau)) {
        for (int x(x1 - creating_size); x < x1 + 1 + creating_size; x++) {
            for (int y(y1 - creating_size); y < y1 + 1 + creating_size; y++) {
                if (isInsideWorld(x, y)) {
                    int num(nbcells_ * y + x);
                    cells_[num] = Kind::Eau;
                    humidify(x, y);
...
void World::EditCreatingSize(bool mode)
{
    if (mode) {
        creating_size++;
    } else {
...
void World::drawCreatingZone(sf::RenderTarget& target) const
{
    Vec2d topLeft(getApp().getCursorPositionInView().x - sizecell_ * creating_size, getApp().getCursorPositionInView().y - sizecell_ * creating_size);
    Vec2d bottomRight(getApp().getCursorPositionInView().x + sizecell_ * creating_size, getApp().getCursorPositionInView().y + sizecell_ * creating_size);
