...
{
    time_ += dt;
    if ((time_.asSeconds() >= getAppConfig()["simulation"]["flower generator"]["delay"].toDouble()) and
        (getAppConfig()["simulation"]["flower generator"]["active"] == true)) {
        reset();
        bool ok;
        do {
            Vec2d p;
