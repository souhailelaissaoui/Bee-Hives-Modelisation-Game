...
public:
    ...
    void drawOn(sf::RenderTarget& target); // dessine le monde dans le parametre target
    ...
    double gethumidity(Vec2d p); // retourne la valeur d'humidité de la cellule dans laquelle se trouve la position graphique passée en argument.
    ~World(); // destructeur
