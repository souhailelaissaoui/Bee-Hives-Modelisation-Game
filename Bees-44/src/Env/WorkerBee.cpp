...
WorkerBee::WorkerBee(Hive* home, Vec2d center):
    Bee(home,center, getAppConfig()["simulation"]["bees"]["worker"]["size"].toDouble()
        , getAppConfig()["simulation"]["bees"]["worker"]["energy"]["initial"].toDouble()
        , getAppConfig()["simulation"]["bees"]["worker"]["speed"].toDouble()
...
void WorkerBee::onState(State state, sf::Time dt)
{
    ...
    if (state == WorkerBee::COLLECTING) {
        ...
        if (fleur == nullptr) {
            ...
            if (fleur->Collecting == nullptr) {
                fleur->Collecting = this;
            }
            if (fleur->Collecting->getHome() == getHome()) {
                ...
                if (isColliding(*(fleur->Collecting))) {
                    Agress(fleur->Collecting);
                };
                if (getEnergie() > 0) {
                    fleur->Collecting = this;
                }
...
void WorkerBee::drawOn(sf::RenderTarget& target) const
{
    ...
    if (isDebugOn() ) {
        ...
        if (this->getState() == COLLECTING) {
            ...
            //next = std::to_string(nectar_) +"/"+ std::to_string(getConfig()["max pollen capacity"].toDouble()) + "]";
        }
