...
public:
    ...
    Hive* getCollidingHive(const Collider& body) const; //retourne une ruche de l'environnement qui est en collision avec le corps circulaire
    Flower* getCollidingFlower(const Collider& body) const; //retourne une fleur de l'environnement qui est en collision avec le corps circulaire
    void drawOn(sf::RenderTarget& target) const override; // methode de dessin principale, va dessiner les fleurs et le monde
    void drawFlowerZone(sf::RenderTarget& target, Vec2d const& position) const; // dessine momentanément le cercle rouge ou vert pour indiquer si il est possible ou non de dessiner une fleur dans la position souhaitee
    void drawHiveableZone(sf::RenderTarget& target, Vec2d const& position) const; //dessine momentanément le carre rouge, bleu ou vert pour indiquer si il est possible ou non de dessiner une ruche dans la position souhaitee
