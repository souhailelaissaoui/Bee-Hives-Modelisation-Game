...
Bee::Bee(Hive* home, Vec2d center, double radius, double energy, double speed, States states)
    ...
    , home_(home)
    , energy_(energy)
    , speed_(speed * Vec2d::fromRandomAngle())
{
}
...
void Bee::getAgressed(double energy)
{
    energy_ -= energy;
}

void Bee::Agress(Bee* other)
{
    if (other != nullptr) {
        if (other->getHome() != home_) {
            other->getAgressed(energy_);
            energy_ -= other->getEnergie();
        }
    }
}
